...
namespace CRUDBlazor.Server.Infrastructure.Controllers
{
    ...
    public class ProductoController : ControllerBase
    {
        ...
        public async Task<IActionResult> HistorialProducto([FromQuery] Paginacion paginacion, [FromQuery]string buscar="" )
        {
            ...
            var historialProductos = _productoRepository.ObtenerHistorialProducto();
            if (!string.IsNullOrEmpty(buscar))
            {
        ...
        {        
            var historialProducto= await _productoRepository.DetalleHistorialProductoId(id);
        ...
        public async Task<IActionResult> DescargarHistorialPDF()
        {
            ...
            var (success, errorMessage, pdfData) = await _productoRepository.DescargarPDF();
            if (!success) 
            { 
                return BadRequest(errorMessage);
            }
        ...
        {
            var (success, errorMessage) = await _productoRepository.EliminarHistorial();
            if (success)
            {
                return Ok(success);
            }
